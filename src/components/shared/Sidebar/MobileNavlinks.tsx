...
export default function MobileNavlinks({ isAdmin = false }: { isAdmin?: boolean }) {
      ...
      {navLinks.map((navLink) => {
        ...
        return <li key={navLink.path}>
          <NavLink
            ...
            className='flex flex-col gap-0.5 group items-center mobile-nav'>
            <div className="relative">
              <navLink.icon size={20} className="mobile-icon" />
            ...
            <span className='text-base-muted label group-hover:text-foreground transition duration-200 text-[10px] font-semibold'>{navLink.label}</span>
          </NavLink>
        </li>
      })}
    </ul>
