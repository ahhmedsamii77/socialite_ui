...
import { useGetAllusers, useGetUserData, useUpdatePost } from "@/lib/apis/queries";
import { Select, SelectContent, SelectGroup, SelectItem, SelectTrigger, SelectValue } from "../ui/select";
import { Globe, Image, Loader2, Lock, MessageCircle, MessageCircleOff, Save, Smile, UserRoundPlus, Users, XIcon } from "lucide-react";
...
export default function EditPostModal({ isOpen, setIsOpen, post }: { isOpen: boolean, setIsOpen: (open: boolean) => void, post: PostType }) {
  ...
  const [allowComments, setAllowComments] = useState<"allow" | "deny">(post?.allowComments!);
  const [files, setFiles] = useState<File[] | null>(null)
  const [searchedTag, setSearchedTag] = useState("");
  const [activeTab, setActiveTab] = useState<string | null>(null);
  const debouncedTag = useDebounce(searchedTag, 100);
  const [tags, setTags] = useState<UserType[]>(post?.tags!);
  const { data: users } = useGetAllusers(debouncedTag !== "" ? debouncedTag.trim() : undefined);
  const [availability, setAvailability] = useState<"public" | "friends" | "only_me">(post?.availability!);
  ...
  useEffect(() => {
    ...
    if (!post?.attachments?.length) return setPreviews(null);
    const previewsData = post.attachments.map((attachment) => {
  ...
  useEffect(() => {
    if (!post?.attachments) return setPreviews(null);
    const previewsData = post.attachments.map((attachment) => {
      const isImg = attachment.match(/\.(jpeg|jpg|gif|png|webp|bmp|tiff|tif|svg|avif|ico)$/i);
      return {
  ...
  function handleRemoveImage(attachment: string, index: number) {
    ...
    setRemovedAttachments((prev) => [...(prev || []) as unknown as string, ...(post?.attachments?.filter((a) => a === sub) as unknown as string[])] as unknown as string[]);
  }
  ...
  const keptAttachments = (post?.attachments?.length ?? 0) - (removedAttachments?.length ?? 0);
  ...
  async function handleUpdatePost() {
    ...
    try {
      ...
      const newTags = tags.filter((t) => !post?.tags?.some((tag) => tag._id === t._id));
      if (newTags?.length) {
        newTags.forEach((t, i) => {
      ...
      if (availability !== post?.availability) formData.append('availability', availability);
      if (allowComments !== post?.allowComments) formData.append('allowComments', allowComments);

      const keptAttachments = (post?.attachments?.length ?? 0) - (removedAttachments?.length ?? 0);
      const hasContentOrAttachments = !!text || !!files?.length || keptAttachments > 0;
      if (!hasContentOrAttachments) {
        toast.error("Post must have at least content or an attachment.");
        setIsLoading(false);
        return;
      }

      await updatePost({ postId: post?._id, data: formData });
  ...
  function handleRemoveTag(user: UserType) {
    ...
    setRemovedTags((prev) => [...(prev || []), ...((post?.tags?.filter((t) => t._id === user._id).map((t) => t?._id)) as unknown as string[])] as unknown as string[]);
  }
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      ">
            ...
            <Select value={availability} onValueChange={setAvailability as any}>
              <SelectTrigger
                ...
                className="cursor-pointer
                 h-6.5! px-2.5 gap-1
              font-semibold border border-[#8B5CF638] bg-[rgba(139, 92, 246, 0.1)]
              hover:bg-[#8B5CF62D]! hover:border-[#8B5CF666] transition duration-200
              rounded-full text-secondary-base bg-[#8B5CF61A]!">
                <SelectValue defaultValue={post?.availability} />
              </SelectTrigger>
        ...
        <div className="px-6 border-t pb-4 pt-3 space-y-2">
          <h3 className="text-[11.5px] uppercase font-semibold tracking-[0.07em] text-base-muted">Edit your post</h3>
          <div className="flex items-center justify-between">
            <Tabs value={activeTab ?? ""}>
              <TabsList className="bg-transparent p-0! md:gap-1 lg:gap-2">
                ...
                <TabsTrigger
                  className={`m-0! ${allowComments == "allow" ? '  hover:bg-[#14B8A61F] hover:text-[#14b8a6]' : 'data-[state=active]:text-[#f87171]!  data-[state=active]:bg-[#EF44441F] hover:bg-[#EF44441F] hover:text-[#f87171]'}  cursor-pointer w-10 h-10 hover:-translate-y-px  transition duration-200 rounded-lg text-base-muted flex items-center justify-center`}
                  value="allowComment"
                  onClick={() => {
